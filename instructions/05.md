...
1. Once connected, the terminal command prompt will change to show the name of the Linux VM, similar to the following.
    ...
    ```cmd/sh
    demouser@vlinuxagentvm-{deployment-id}:~$
    ```
...
1. On the **turbine-06** blade, under **Modules**, notice that **tempsensor** is now listed.
...
1. To view the module logs, enter the following command:
    ...
    ```cmd/sh
     demouser@linuxagentvm-{deployment-id}:~$ iotedge logs tempsensor
...
1. Replace the default route defined with the following three routes:
    ...
    * Route 2
        * NAME: **alertsToReset**
        * VALUE: `FROM /messages/modules/iotedge-streamjob-{deployment-id}/* INTO BrokeredEndpoint("/modules/tempsensor/inputs/control")`
    * Route 3
        * NAME: **telemetryToAsa**
        * VALUE: `FROM /messages/modules/tempsensor/* INTO BrokeredEndpoint("/modules/iotedge-streamjob-{deployment-id}/inputs/temperature")`
    ...
    "$edgeHub": {
        "properties.desired": {
            "routes": {
                ...
                "alertsToReset": "FROM /messages/modules/iotstreamjob-{deployment-id}/* INTO BrokeredEndpoint(\\\"/modules/tempsensor/inputs/control\\\")",
