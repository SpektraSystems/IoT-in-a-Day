...
When you have a huge number of devices, a large amount of data is being collected and sent to the cloud. Is it possible to bring intelligence to the Edge?
...
* Connect to IoT Edge VM (Simulating an Edge Device)
* Add an Edge Module to the Edge Device
* Deploy Azure Stream Analytics as an IoT Edge Module

Here is the architecture for your review:

![Lab 04 Architecture](media/lab4_architecture.png)

### Task 1: Connect To IoT Edge VM

In this task, you will connect to the IoT Edge VM. This VM is simulating an IoT Edge device. 
...
1. Once connected, the terminal command prompt will change to display the name of the Linux VM, similar to the following.
    ...
    > **Important:** When you connect, you will likely be told there are outstanding OS updates for the Edge VM.  We are ignoring this for our lab purposes, but in production, you would always want to be sure to keep your Edge devices up-to-date.
...
1. If necessary, log in to your Azure portal using lab Azure account credentials.

1. On your Resource group tile, to open your IoT Hub, click **iothub-{deployment-id}**.

1. In **IoT Hub** blade, under **Automatic Device Management**, click **IoT Edge**.
...
1. In **turbine-06** blade, click **Set Modules**.
...
1. Under **Image URI**, enter **asaedgedockerhubtest/asa-edge-test-module:simulated-temperature-sensor**
    ...
    > **Note**: This image is a published image on Docker Hub and is provided by the product group to support this testing scenario.
...
1. In **Deployment** page, take a minute to review the deployment manifest that is displayed. 

    As you can see, the Deployment Manifest for the IoT Edge Device is formatted as JSON, which makes it pretty easy to understand.
    ...
    Subsequently **$edgeHub** section that contains the desired properties for the Edge Hub. This section also includes the routing configuration for routing events between modules, and to IoT Hub.
    ...
    ```
     ...
     In the end, **turbinesensor** module is defined, where the `properties.desired` section contains the desired properties for the configuration of the edge module.
...
1. Click **Create** to finish setting the modules for the device
...
1. Notice that the **RUNTIME STATUS** for the **turbinesensor** module is now set to **running**.
    ...
    If the value is still not updated, please wait for a few minutes and refresh the blade again.
 
1. Open a Cloud Shell session (if it is not still open).

    If you are no longer connected to the `vm-iot-edge-{deployment-id}` virtual machine, connect using **SSH** as instructed earlier.

1. In the Cloud Shell ,  list the modules currently running on the IoT Edge Device by running the following command:
...
Now that the **turbinesensor** module is deployed and running on the IoT Edge device, we can add a Stream Analytics module that can process messages on the IoT Edge device before sending them to the IoT Hub.
...
1. Under Job Topology on the left side of the blade, select Inputs and verify that there is one input job **temperature** already defined.
...
1. In **turbine-06** pane, click **Set Modules**.
...
1. In the **Edge job** dropdown, ensure that the **iotedge-streamjob-{deployment-id}** Steam Analytics job is selected.
    ...
    > **Note**:  The job may already be selected, yet the **Save** button is disabled - just open the **Edge job** dropdown again and select the **iotedge-streamjob-{deployment-id}** job again. The **Save** button should become enabled now.
...
1. On the **Update IoT Edge Module** pane, notice that the **Image URI** points to a standard Azure Stream Analytics image.
    ...
    > **Note**:  The version number at the end of the **Image URI** reflects the latest version of the time when you created the Stream Analytics Module.
...
1. Replace the default route defined with the following  routes:
...
1. In **Set modules on device: turbine-06** blade, click **Review + create**.
...
1. Review the JSON configuration for the `turbinesensor` Simulated Temperature Sensor module:
...
1. Review the JSON configuration for the routes that were previously configured, and how they are configured in the JSON Deployment definition:
...
1. Navigate to the **Cloud Shell** session where you're connected to the **IoT Edge Device** over **SSH**.  
...
1. At the command prompt, to view a list of the modules deployed to the device, enter the following command:
    ...
    It can take a minute for the new Stream Analytics module to be deployed to the IoT Edge Device. Once it's there, you will see it in the list.
    ...
    > **Note**:  If the Stream Analytics module does not show up in the list, wait for a few minutes and try again. It can take a few minute for the module deployment to be updated on the IoT Edge Device.

1. Run the following command to watch the telemetry being sent from the Edge device by the `turbinesensor` module, 
...
1. Take a minute to observe the output.
    ...
    While watching the temperature telemetry being sent by **turbinesensor**, notice that a **reset** command is sent by the Stream Analytics job when the `machine.temperature` reaches an average above `25`. This is the action configured in the Stream Analytics job query.
