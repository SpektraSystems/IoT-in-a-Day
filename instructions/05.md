...
1. At the Cloud Shell command prompt, customize and then run the following command: 
    ...
    `sudo ./InstallSecurityAgent.sh -i -aui Device -aum SymmetricKey -f /home/demouser/s.key -hn iot-az220-training-ab200213.azure-devices.net -di rodpump-01`
...
1. Under **Module Identities**, notice that your **azureiotsecurity** module is now in a **Connected** state.
    ...
    ![Screenshot of Azure IoT Security Module Connected](media/LAB_05_device-connected-agent.png)
...
1. At the top of the **default** (Device Security Group) blade, click **Save**.
    ...
    Without saving the new alert, the alert is deleted the next time you close IoT Hub. You will be returned to the list of custom alerts. Below is an image that shows a number of custom alerts:

### Exercise 7: IoT Device with Device Twin
