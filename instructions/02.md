...
1. From the **Overview** blade of your **Stream Analytics job**, take a few minutes to observe the query used here: 
 ...
 > **Note**: **SELECT * INTO bloboutput FROM iothubinput;** - Here we will select all the telemetry data from the IoThUb into the storage account.
 > **Note**: **SELECT AVG(temp) AS Average, id  INTO powerbioutput  FROM iothubinput  GROUP BY TumblingWindow(minute, 1), id;** - Here we will average value of temperature from the telemetry data from the IoT Hub into the Power BI . 
