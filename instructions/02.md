...
In this task, we will run a predefined **Stream Analytics Job** to process and transform the data from IoT Hub to a storage account where all the "cold" data is stored into scalable storage so that they can summarize it periodically and also to PowerBI to visualize the "hot" data showing the average temperature reported by each device over a 1-minute window. 
...
**Azure Time Series Insights (TSI)** is an end-to-end **platform-as-a-service** offering used to collect, process, store, analyze, and query data from IoT solutions at scale. TSI is designed for ad hoc data exploration and operational analysis of data that are highly contextualized and optimized for time series.
...
1. Once logged in to the TSI explorer pane, you will see as shown below:
   ...
   > **Note**: Sometimes the Time Series Insight Explorer will display "There is no environment available", in that case click on *Check out a sample* option from the list and continue from step 6 .
