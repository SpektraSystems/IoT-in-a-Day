...
In this task we will run a predefined **Stream Analytics Job** to store the data from the IoT Hub to storage account and also to **PowerBI**
...
1. From the **Overview** blade of your **Stream Analytics job**, take a few minutes to observe the query used here: 
 ...
 > **Note**: **SELECT * INTO bloboutput FROM iothubinput;** - Here we will select all the telemetry data from the IoT Hub into the storage account.

 > **Note**: **SELECT AVG(temp) AS Average, id  INTO powerbioutput  FROM iothubinput  GROUP BY TumblingWindow(minute, 1), id;** - In this we select average value of temperature from the telemetry data from the IoT Hub into the Power BI . 

> **Note**: The data saved in the storage account is processed data and that can also be done through Message Routing . 
 Please take some time to explore through the process of message routing, https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-devguide-messages-d2c
...
> **Note**: The data saved in the storage account is processed data and that can also be done through **Message Routing**. 
 Please take some time to explore through the process of message routing, https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-devguide-messages-d2c
 Depending on the needs, we can also save processed data (Average) in ASA , For example: **SELECT AVG(temp) INTO bloboutput FROM iothubinput;** can be used to store average temperature as telemetry data from IoT Hub to storage account.
