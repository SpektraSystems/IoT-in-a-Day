...
1. Once logged in to the TSI explorer pane, you will see as shown below:
   ...
   > **Note**: Sometimes the Time Series Insight Explorer window tab will display "There is no environment available", in that case click on *Check out a sample* option from the list and continue from step 6 as the data from the IoT Hub might take some time to get loaded into TSI Environment.
