...
### Exercise 3: Connect devices (simulated) to Azure IoT Hub
...
1. Click on **Register** which is next to the **rodpump-01** on the IoT Simulator app dialog to register device to the IoThub.
...
1. If you got a prompt for creating storage account, click on **Show advanced settings** and select existing Resource Group as **iot-{deployment-id}** and enter **iotstorage{deployment-id}** for storage account name and Enter **blob** for File Share 
...
1. Notice that the `az iot hub monitor-events` Azure CLI command outputs a JSON representation of the events that are arriving at your specified Azure IoT Hub. 
    ...
    ```cmd/sh
    Starting event monitor, filtering on device: rodpump-01, use ctrl-c to stop...
    {
        "event": {
            "origin": "rodpump-01",
            "module": "",
            "interface": "",
            "component": "",
            "payload": "{\"id\":\"rodpump-01\",\"time\":\"2020-08-14T12:50:48.4702636Z\",\"temp\":69.0}"
        }
    }
    {
        "event": {
            ...
            "interface": "",
            "component": "",
            "payload": "{\"id\":\"rodpump-01\",\"time\":\"2020-08-14T12:48:21.7545040Z\",\"temp\":70.0}"
        }
    }
    ```
