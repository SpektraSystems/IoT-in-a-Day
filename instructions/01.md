...
* Login to Azure Portal and verify the pre-deployed resources in your lab environment. 
...
1. To get the lab environment details, you can select the **Lab Environment** tab on the right. Additionally, the credentials will also be emailed to your email address provided during registration.
 ...
 > You will see Unique DeploymentID value on **Lab Environment** tab, use it wherever you see SUFFIX or DeploymentID in lab steps.
...
1. On **Sign in to Microsoft Azure** blade, enter following username and then click on **Next**. 
...
1. In the upper left corner of the portal, click the hamburger menu icon to open the Azure portal menu,
    ...
    * The **All services** button opens up a list of all services offered by Azure.

1. In the Azure portal menu, click **Resource groups**

    This blade displays all of the resource groups that you have access in the lab Azure subscription.
...
1. Minimize the azure portal and open the **IoT Simulator App** by clicking on the **IoT simulator** shortcut from your lab virtual machine desktop. 
...
1. Minimize the **IoT Hub Simulator App** and navigate to the **Iot hub** in Azure Portal.

1. Click **IoT devices** under Explorers section and you can see that turbine devices are now registered. 
...
1. Click on **Activate** to simulate changing the device status from disabled to enabled in the IoT Hub Registry.
   ...
   > **Note: Repeat the above steps to register and activate all the turnbine devices**. 
                  
1. At this point, you have registered and activated devices. To view this list of devices, Switch over to the Azure Portal and select the IoT Hub **iothub-{deployment-id}** .
...
In this task, you will use the Azure CLI to verify telemetry sent by the simulated devices to the IoT Hub.
...
1. In the Azure Cloud Shell, to monitor the event messages  being received by your IoT hub, enter the following command:
    ...
    The `monitor-events` command (within the `az iot hub` Azure CLI module) offers the capability to monitor device telemetry and other message types sent to an Azure IoT Hub. This can be a very useful tool during development, and the convenience of the command-line interface is also nice.
...
1. Notice that the `az iot hub monitor-events` Azure CLI command outputs a JSON representation of the events that are arriving at your specified Azure IoT Hub. 
    ...
    You should see messages displayed similar to the following:
