...
1. At the command prompt, to create an IoT Edge device identity in your IoT hub, enter the following command:
    ...
    ```cmd/sh
    az iot hub device-identity create --hub-name iot-in-a-day-hub-{deployment-id} --device-id sensor-th-0067 --edge-enabled
    ```

    Be sure to replace the `{deployment-id}` placeholder with the **DeploymentID** value provided on the Environment Details Tab under **Lab Environment**.
...
1. To display the **Connection String** for your IoT Edge device, enter the following command:
    ...
    ```cmd/sh
    az iot hub device-identity show-connection-string --device-id sensor-th-0067 --hub-name iot-in-a-day-hub-{deployment-id}
    ```

    > **Note**: Be sure to replace the `{deployment-id}` with **Deployment ID**. You can obtain the **Deployment ID** from the Environment Details Tab.
...
1. Once connected, the terminal command prompt will change to show the name of the Linux VM, similar to the following.
    ...
    ```cmd/sh
    demouser@vm-iot-edge-{deployment-id}:~$
    ```
...
1. On your Resource group tile, to open your IoT Hub, click **iot-in-a-day-hub-{deployment-id}**.
...
1. The output of the command look similar to the following. 
    ...
    ```cmd/sh
    demouser@vm-iot-edge-{deployment-id}:~$ iotedge list
...
1. To view the module logs, enter the following command:
    ...
    ```cmd/sh
     demouser@vm-iot-edge-{deployment-id}:~$ iotedge logs tempsensor
...
#### Task 1: Review the Stream Analytics Job

1. On the resource group tile, click iot-{deployment-id} and select the stream analytics job named iotstreamjob-edge-{deployment-id}.
...
#### Task 2: Deploy the Stream Analytics Job

1. In the Azure portal, navigate to your **iot-in-a-day-hub-{deployment-id}** IoT Hub resource.
...
1. In the **Edge job** dropdown, ensure that the **iotstreamjob-edge-{deployment-id}** Steam Analytics job is selected.

    > **Note**:  The job may already be selected, yet the **Save** button is disabled - just open the **Edge job** dropdown again and select the **iotstreamjob-edge-{deployment-id}** job again. The **Save** button should then become enabled.
...
1. Under **IoT Edge Modules**, click **iotstreamjob-edge-{deployment-id}**.. 
...
1. Replace the default route defined with the following three routes:
    ...
    * Route 2
        * NAME: **alertsToReset**
        * VALUE: `FROM /messages/modules/iotstreamjob-{deployment-id}/* INTO BrokeredEndpoint("/modules/tempsensor/inputs/control")`
    * Route 3
        * NAME: **telemetryToAsa**
        * VALUE: `FROM /messages/modules/tempsensor/* INTO BrokeredEndpoint("/modules/aiotstreamjob-{deployment-id}/inputs/temperature")`

    > **Note**: Be sure to replace the `iotstreamjob-edge-{deployment-id}` placeholder with the name of your Azure Stream Analytics job module. You can click **Previous** to view the list of modules and their names, then click **Next** to come back to this step.
...
1. At the command prompt, to view a list of the modules deployed to the device, enter the following command:
    ...
    ```cmd/sh
    demouser@vm-vm-iot-edge-{deployment-id}:~$ iotedge list
    NAME               STATUS           DESCRIPTION      CONFIG
    vm-iot-edge-745896 running          Up a minute      mcr.microsoft.com/azure-stream-analytics/azureiotedge:1.0.5
