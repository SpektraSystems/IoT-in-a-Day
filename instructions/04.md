...
Upon anomaly detection, Your team decides that further improvements to the monitoring system will require near real-time data exploration and alerting
...
1. On the **Datasets** tab, verify that **temperaturedataset** is displayed.
    ...
    If not, you might have to wait for few mninutes for this list to populate.

1. At the top right of the page, click **+ Create**, and then click **Dashboard**.

1. In the **Create dashboard** popup, under **Dashboard name**, type **Temperature Dashboard** and then click **Create**.

    The will open up dashboard designed.

1. To add a vibration gauge, at the top of the empty dashboard, click **+ Add tile**.
...
1. On the **Add a custom streaming data tile** pane, under **YOUR DATASETS**, select **temperaturedataset**, and then click **Next**.
...
Now Let's create a fourth tile for `Anomalies Over the Hour` line chart.  This one includes a bit more configuration than the previous ones.
...
### Task 2: Configure alert notifications when an anomaly is detected [Optional]
...
1. Click **+ New Step** on your process, then type in **Common Data Services**, Select **Common Data Services** and then select **Get record** action.
   ...
   **Note**: You will not be able to perform the remaining steps using the CloudLabs provided credentials, but can take a look at the remaining steps to understand how to configure alerts.
